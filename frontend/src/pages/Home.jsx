...
const AnimatedNumber = ({ value }) => {
  ...
  // Extract number from string (e.g., "10000+" -> 10000)
  const numericValue = parseInt(value.replace(/\D/g, '')) || 0;

  // Animation settings for SLOW increment
  ...
  const displayValue = useTransform(springValue, (current) =>
    Math.round(current).toLocaleString() + (value.includes('+') ? '+' : '')
  );
...
const HomePage = () => {
  const stats = [
    { label: 'Countries', value: '50+', icon: Globe, color: 'from-blue-500 to-blue-600',  desc: 'Global Reach' },
    { label: 'Members', value: '10000+', icon: Users, color: 'from-green-500 to-green-600', desc: 'Strong Community' },
    { label: 'Events', value: '200+', icon: Calendar, color: 'from-purple-500 to-purple-600',  desc: 'Cultural Programs' },
    { label: 'Awards', value: '100+', icon: Award, color: 'from-orange-500 to-orange-600',  desc: 'Excellence Recognized' },
  ];
  return (
      ...
      <section className="pt-36 pb-20 bg-gradient-to-br from-orange-50 via-white to-blue-50 relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
            {stats.map((stat, idx) => (
              ...
              >
                {/* Main Card */}
                <div className={`relative h-full bg-gradient-to-br ${stat.color} p-8 rounded-[2.5rem] ${stat.shadow} shadow-2xl transition-all duration-500 hover:scale-[1.05] hover:-translate-y-2 overflow-hidden border border-white/20`}>
                  ...
                  <stat.icon className="absolute -right-4 -bottom-4 w-28 h-28 opacity-10 rotate-12 group-hover:rotate-45 transition-transform duration-700" />

                  <div className="relative z-10 flex flex-col items-center text-center">
                    {/* Icon Container */}
                    <div className="mb-5 p-4 bg-white/20 rounded-2xl backdrop-blur-md border border-white/30 shadow-inner group-hover:scale-110 transition-transform">
                      <stat.icon className="w-10 h-10 text-white" />
                    </div>

                    {/* Animated Counter */}
                    <h3 className="text-4xl lg:text-5xl font-black text-white mb-2 tracking-tighter drop-shadow-sm">
                      <AnimatedNumber value={stat.value} />
                    </h3>

                    {/* Labels */}
                    <div className="text-sm font-bold text-white/90 mb-1 tracking-widest uppercase">
                      {stat.label}
                    </div>
          ...
          <motion.div
            ...
            variants={{
              ...
              show: {
                opacity: 1,
                transition: {
                  staggerChildren: 0.2 // Ek card aur dusre card ke beech ka delay
                }
              }
            }}
          ...
          >
            {[
              ...
              <motion.div
                ...
                whileHover={{ y: -10, transition: { duration: 0.2 } }} // Hover par halka sa lift
                className={`${item.bgColor} p-8 rounded-3xl border-2 border-transparent hover:border-orange-500 transition-shadow duration-300 hover:shadow-2xl relative group`}
              >
                {/* Icon Animation */}
                <div className={`w-16 h-16 ${item.bgColor} rounded-2xl flex items-center justify-center mb-6 border-2 ${item.color.replace('text', 'border')} group-hover:scale-110 transition-transform duration-300`}>
                  <item.icon className={`w-8 h-8 ${item.color}`} />
                </div>
                ...
                <div className="absolute bottom-0 left-8 right-8 h-1 bg-gradient-to-r from-transparent via-orange-500/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
              </motion.div>
            ))}
          </motion.div>
        </div>
